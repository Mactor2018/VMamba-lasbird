...
if [ ! -f "/root/labirds/complementaryExperiments/open_clip/scripts/bird_train_pure.csv" ]; then
    echo "错误: 训练集CSV文件不存在!"
    exit 1
fi

if [ ! -f "/root/labirds/complementaryExperiments/open_clip/scripts/bird_test_pure.csv" ]; then
    echo "错误: 测试集CSV文件不存在!"
    exit 1
fi
...
python -m torch.distributed.launch \
    ...
    --output ${OUTPUT_DIR} \
    --tag "single_a800_$(date +%Y%m%d_%H%M%S)" \
